...
export const useActivites=(id?: string)=>
{
  ...
  //select transform the data before send it back
  ...
  const createActivity = useMutation({
    ...
    const updateAttendance = useMutation({
        ...
        onMutate: async (activityId: string) => {
            await clientQuery.cancelQueries({queryKey: ['activities', activityId]});

            const prevActivity = clientQuery.getQueryData<Activity>(['activities', activityId]);

            clientQuery.setQueryData<Activity>(['activities', activityId], oldActivity => {
        ...
        onError: (error, activityId, context) => {
            ...
            if (context?.prevActivity) {
                clientQuery.setQueryData(['activities', activityId], context.prevActivity)
            }
        }
    })
